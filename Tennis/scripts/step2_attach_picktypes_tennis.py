#!/usr/bin/env python3
"""
Tennis step2 — normalize pick types, prop_norm, infer tour (ATP/WTA) from rankings match.
"""
...
def main() -> None:
    ...
    if df.empty:
        print("ERROR [Tennis-S2] empty step1")
        sys.exit(1)
    ...
    df["unsupported_prop"] = (~df["prop_norm"].isin(["games_won", "match_total_games"])).astype(int)

    out.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(out, index=False, encoding="utf-8-sig")
    print(f"OK [Tennis-S2] -> {out}  rows={len(df)}")
