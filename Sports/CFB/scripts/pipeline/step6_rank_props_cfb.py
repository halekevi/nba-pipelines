#!/usr/bin/env python3
"""
cfb_step6_rank_props.py  (v3 — under-direction fixes)
-------------------------------------------------------
Ranks CFB props using the same signal set as NBA step7:
...
_repo_root_cbb = Path(__file__).resolve().parents[4]
...
from utils.cfb_playoff_metadata import (
    ...
    CFB_PLAYOFF_TD_SCORE_MULT,
    CFB_PLAYOFF_YARD_SCORE_MULT,
    cfb_is_championship_row,
    cfb_playoff_info,
    cfb_row_in_playoff,
)

# CFB ML cap (same rationale as CBB — avoid 0.99x overconfidence).
CFB_ML_PROB_MAX = 0.80
...
_PROP_WEIGHTS = {
    "pass_yds": 1.02, "rush_yds": 1.04, "rec_yds": 1.03,
    "pass_td": 1.05, "rush_td": 1.06, "rec_td": 1.05,
    "rec": 1.04, "int": 0.90, "fantasy": 0.88,
}
...
_PROP_HIT_RATE_PRIOR = {
    "pass_yds": 0.55, "rush_yds": 0.54, "rec_yds": 0.53,
    "pass_td": 0.52, "rush_td": 0.51, "rec_td": 0.50,
    "rec": 0.54, "int": 0.48, "fantasy": 0.52,
}
...
def _apply_ml_blend(out: pd.DataFrame, existing_score: pd.Series, source_hint: str = "") -> tuple[pd.Series, pd.Series, pd.Series]:
    ...
    ml_prob = pd.to_numeric(ml_prob, errors="coerce").clip(lower=0.001, upper=CFB_ML_PROB_MAX)
    ml_edge = ml_prob - 0.5
...
def _pick_cfb_boxscore_cache(explicit: str, input_csv: str) -> str:
    """Prefer the largest on-disk CFB boxscore CSV so H2H sees full history."""
    import os
    from pathlib import Path
    ...
    cands.append(root / "data" / "cache" / "cfb_boxscore_cache.csv")
    cands.append(root / "cfb_boxscore_cache.csv")
    seen: set[str] = set()
    uniq: list[Path] = []
    for p in cands:
        try:
...
def main():
    ...
    ap.add_argument("--output",     default="step6_ranked_cfb.xlsx")
    ...
    _repo_usage = Path(__file__).resolve().parents[4]
    ...
    _repo_cbb = Path(__file__).resolve().parents[4]
    ...
    _inj_pen_cbb = (
        penalty_series_for_slate(out, _player_inj, _team_inj, "CFB", _inj_cbb_path)
        if _inj_cbb_path
    ...
    _team_tr = next((c for c in ("team", "pp_team", "team_abbr", "Team") if c in out.columns), None)
    _opp_tr = next(
        (c for c in ("opp", "opp_team_abbr", "pp_opp_team", "Opp") if c in out.columns),
        None,
    )
    if _team_tr and _opp_tr:
        is_po_s = pd.Series(
            [
                cfb_row_in_playoff(out.iloc[i][_team_tr], out.iloc[i][_opp_tr])
                for i in range(len(out))
            ],
            index=out.index,
        )
        po_round = pd.Series(
            [
                cfb_playoff_info(out.iloc[i][_team_tr])[1]
                or cfb_playoff_info(out.iloc[i][_opp_tr])[1]
                for i in range(len(out))
            ],
            index=out.index,
        )
        po_seed = pd.Series(
            [
                cfb_playoff_info(out.iloc[i][_team_tr])[0]
                or cfb_playoff_info(out.iloc[i][_opp_tr])[0]
                for i in range(len(out))
            ],
            index=out.index,
        )
        is_champ = pd.Series(
            [
                cfb_is_championship_row(out.iloc[i][_team_tr], out.iloc[i][_opp_tr])
                for i in range(len(out))
            ],
            index=out.index,
        )
    else:
        is_po_s = pd.Series(False, index=out.index)
        po_round = pd.Series("", index=out.index)
        po_seed = pd.Series("", index=out.index)
        is_champ = pd.Series(False, index=out.index)

    out["is_playoff_game"] = is_po_s.astype(int)
    out["is_tournament_game"] = out["is_playoff_game"]
    out["playoff_round"] = po_round
    out["team_playoff_seed"] = po_seed

    pn_lower = prop_norm_col.astype(str).str.lower().str.strip()
    yard_like = pn_lower.isin(("pass_yds", "rush_yds", "rec_yds"))
    td_like = pn_lower.isin(("pass_td", "rush_td", "rec_td"))
    raw_score = raw_score * np.where(is_po_s & yard_like, CFB_PLAYOFF_YARD_SCORE_MULT, 1.0)
    raw_score = raw_score * np.where(is_po_s & td_like, CFB_PLAYOFF_TD_SCORE_MULT, 1.0)
    raw_score = raw_score * np.where(is_po_s & is_champ, CFB_PLAYOFF_CHAMP_EXTRA_MULT, 1.0)
    ...
    _apply_consistency_grade_scores_cbb(out, "CFB")

    # Game script risk adjustment
    _root_gs = Path(__file__).resolve().parents[4]
    ...
    if player_col and opp_col:
        cache_for_h2h = _pick_cfb_boxscore_cache(args.cache, args.input)
        out = _attach_h2h(
            out,
            cache_for_h2h,
            "cbb",
