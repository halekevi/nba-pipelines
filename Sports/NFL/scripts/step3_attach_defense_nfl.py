...
DB_TABLE_NAME = "nfl_defense_summary"


def main() -> int:
    ap = argparse.ArgumentParser(description="NFL step3 scaffold")
    ap.add_argument("--input", required=True)
    ap.add_argument("--output", required=True)
    args = ap.parse_args()

    df = pd.read_csv(args.input) if Path(args.input).is_file() else pd.DataFrame()
    if "defense_rank" not in df.columns:
        df["defense_rank"] = pd.NA
    out = Path(args.output)
    out.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(out, index=False, encoding="utf-8-sig")
    print(f"[NFL step3] rows={len(df)} -> {out}")
    return 0
