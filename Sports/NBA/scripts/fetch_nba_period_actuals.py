...
def _nba_game_ids_for_date(date_str: str) -> list[str]:
    print(f"Using ScoreboardV3 for game IDs (date={date_str})")
    ...
    try:
        sb = ScoreboardV3(game_date=f"{mm}/{dd}/{yyyy}", timeout=NBA_SCOREBOARD_TIMEOUT)
        frames = sb.get_data_frames()
    except Exception:
        return []
    # V3 game list: frame 1 uses camelCase gameId (V2 GameHeader used GAME_ID).
    if len(frames) < 2:
        return []
    games_df = frames[1]
    ...
    out = [
        str(gid).strip()
        for gid in games_df["gameId"].dropna().astype(str).unique()
        if str(gid).strip()
    ]
