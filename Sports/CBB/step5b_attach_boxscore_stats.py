...
def parse_players(summary: dict, game_date: str = "", event_id: str = "") -> List[dict]:
    ...
    for tb in blocks:
        ...
        i["3PM"] = next((idx(lbl) for lbl in ("3PM","FG3M","3PT","3PTM","3FGM") if lbl in ul), None)
        ...
        for a in athletes:
            ...
            if pn:
                rows.append({"team_id": team_id, "opp_team_id": opp_team_id,
                             "player_norm": pn,
                             "espn_athlete_id": aid,
                             "game_date": game_date,
                             "event_id":  event_id,
    ...
    if cache_path and os.path.exists(cache_path):
        try:
            ...
            for rr in cached_rows:
                ...
                has_opp = "opp_team_abbr" in rr and str(rr.get("opp_team_abbr","")).strip() not in ("", "nan")
                for col in ("MIN","PTS","REB","AST","STL","BLK","TO","3PM"):
                    try: rr[col] = float(rr[col])
                    except Exception: rr[col] = 0.0
                # Mark stale if missing opp_team_abbr (H2H fix)
                if not has_opp:
                    stale_eids.add(str(rr.get("event_id","")))

            # Mark events where every player has 3PM=0 as stale — indicates label parse failure.
            # (Genuine all-zero-3PM events are rare and will be cleanly refetched.)
            from collections import defaultdict as _dd
            _ev_3pm = _dd(list)
            for rr in cached_rows:
                eid = str(rr.get("event_id",""))
                if eid:
                    _ev_3pm[eid].append(float(rr.get("3PM") or 0))
            for eid, vals in _ev_3pm.items():
                if vals and sum(vals) == 0.0 and len(vals) >= 5:
                    stale_eids.add(eid)
...
def main():
    ...
    hist_aid, hist_name = build_player_histories(args.days, slate_ids, workers=args.workers,
                                                  cache_path=cache_path, tid_to_abbr=tid_to_abbr)
