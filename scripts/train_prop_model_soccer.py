...
def _load_soccer_frames() -> tuple[pd.DataFrame, list[str]]:
    ...
    if not syn_raw.empty:
        frames.append(_soccer_synthetic_to_workbook_like(syn_raw))
        sources.append("synthetic_graded.db::synthetic")
    if not frames:
        raise FileNotFoundError(
            "No graded_soccer*.xlsx with result+edge under outputs/ or Soccer/, "
            "and no Soccer rows in data/cache/synthetic_graded.db."
        )
...
def main() -> None:
    print("-> Collecting Soccer graded workbooks...")
    df, sources = _load_soccer_frames()
